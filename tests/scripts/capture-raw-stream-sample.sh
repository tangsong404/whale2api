...
API_KEY="${5:-}"
ADMIN_KEY="${WHALE2API_ADMIN_KEY:-admin}"
...
cleanup() {
  ...
  pkill -f "cmd/whale2api" >/dev/null 2>&1 || true
}
trap cleanup EXIT

export WHALE2API_DATABASE_PATH="${WHALE2API_DATABASE_PATH:-docker-data/whale2api/whale2api.db}"

WHALE2API_DEV_PACKET_CAPTURE=1 \
WHALE2API_DEV_PACKET_CAPTURE_LIMIT=20 \
  go run ./cmd/whale2api >/tmp/whale2api_capture_server.log 2>&1 &
...
for line in headers:
  if line.lower().startswith('x-whale2-sample-dir:'):
...
headers=pathlib.Path(sys.argv[1]).read_text().splitlines()
for line in headers:
  if line.lower().startswith('x-whale2-sample-id:'):
    print(line.split(':',1)[1].strip())
    raise SystemExit
print('')
PY
