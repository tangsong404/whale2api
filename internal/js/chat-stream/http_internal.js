...
function internalSecret() {
  return asString(process.env.WHALE2API_VERCEL_INTERNAL_SECRET) || asString(process.env.WHALE2API_ADMIN_KEY) || 'admin';
}
...
function buildInternalGoHeaders(req, opts = {}) {
  const headers = {
    ...
    'x-whale2-target-account': asString(header(req, 'x-whale2-target-account')),
    'x-vercel-protection-bypass': resolveProtectionBypass(req),
  };
  if (opts.withInternalToken) {
    headers['x-whale2-internal-token'] = internalSecret();
  }
...
function resolveProtectionBypass(req) {
  ...
  return asString(process.env.VERCEL_AUTOMATION_BYPASS_SECRET) || asString(process.env.WHALE2API_VERCEL_PROTECTION_BYPASS);
}
